import 'package:hbk/Data/AppData/app_preferences.dart';

const String baseUrl = '';

const String aboutUsUrl =
    'http://sap.al-imdad.com:44891/Picture/CustomerApplogin/about.json';
const String categoryUrl =
    'http://imtxt.sbsolutions.com.pk:44891/api/Categories';
const String dashUrl =
    'http://imtxt.sbsolutions.com.pk:44891/api/lookupapi/CustomerDashboard?CardCode=';

///not used
const String newArrivalProductUrl =
    'http://imtxt.sbsolutions.com.pk:44891/api/lookupapi/GetAllNewArrivalsOpen?code=all';

///
const String rewardUrl =
    'http://imtxt.sbsolutions.com.pk:44891/api/lookupapi/CustomerReward?cardcode=';
const String statementUrl =
    'http://imtxt.sbsolutions.com.pk:44891/api/lookupapi/CustomerStatement?cardcode=';
const String newArrivalGuest =
    'http://imtxt.sbsolutions.com.pk:44891/api/GetAllNewArrivalsGuest';
const String newArrivalAuth =
    'http://imtxt.sbsolutions.com.pk:44891/api/GetAllNewArrivalsAuth?code=all';
const String contactUsUrl =
    'http://sap.al-imdad.com:44891/Picture/CustomerApplogin/Contact.json';
const String allProductsUrl =
    'http://imtxt.sbsolutions.com.pk:44891/api/GetAllItems';
const String allProductGuestUrl =
    'http://imtxt.sbsolutions.com.pk:44891/api/GetAllItemsGuest';
const String categoryProductUrl =
    'http://imtxt.sbsolutions.com.pk:44891/api/GetAllCategoryItems?Code=';
const String productAuthUrl =
    'http://imtxt.sbsolutions.com.pk:44891/api/GetAllItemsAuth?code=';
const String faqUrl =
    'http://sap.al-imdad.com:44891/Picture/CustomerApplogin/faq.json';

const String baseUrl = 'http://imtxt.sbsolutions.com.pk:44891';
const String aboutUsUrl =
    'http://sap.al-imdad.com:44891/Picture/CustomerApplogin/about.json';
const String categoryUrl =
    'http://imtxt.sbsolutions.com.pk:44891/api/Categories';
const String dashUrl =
    'http://imtxt.sbsolutions.com.pk:44891/api/lookupapi/CustomerDashboard?CardCode=';
const String newArrivalProductUrl =
    'http://imtxt.sbsolutions.com.pk:44891/api/lookupapi/GetAllNewArrivalsOpen?code=all';

const String rewardUrl =
    'http://imtxt.sbsolutions.com.pk:44891/api/lookupapi/CustomerReward?cardcode=';
const String statementUrl =
    'http://imtxt.sbsolutions.com.pk:44891/api/lookupapi/CustomerStatement?cardcode=';
const String newArrivalGuest =
    'http://imtxt.sbsolutions.com.pk:44891/api/GetAllNewArrivalsGuest';
const String newArrivalAuth =
    'http://imtxt.sbsolutions.com.pk:44891/api/GetAllNewArrivalsAuth?code=all';
const String contactUsUrl =
    'http://sap.al-imdad.com:44891/Picture/CustomerApplogin/Contact.json';
const String allProductsUrl =
    'http://imtxt.sbsolutions.com.pk:44891/api/GetAllItems';
const String allProductGuestUrl =
    'http://imtxt.sbsolutions.com.pk:44891/api/GetAllItemsGuest';
const String categoryProductUrl =
    'http://imtxt.sbsolutions.com.pk:44891/api/GetAllCategoryItems?Code=';
const String productAuthUrl =
    'http://imtxt.sbsolutions.com.pk:44891/api/GetAllItemsAuth?code=';
const String faqUrl =
    'http://sap.al-imdad.com:44891/Picture/CustomerApplogin/faq.json';

const String notificationUrl =
    'http://imtxt.sbsolutions.com.pk:44891/api/lookupApi/GetAllAlerts?CardCode=c00459';

const String imageUrl =
    'http://imtxt.sbsolutions.com.pk:44891/Picture/CustomerApplogin/';

const String readNotificationUrl =
    'http://imtxt.sbsolutions.com.pk:44891/api/lookupApi/postingAlert';

const String rewardUrl='http://imtxt.sbsolutions.com.pk:44891/api/lookupapi/CustomerReward?cardcode=';
const String statementUrl='http://imtxt.sbsolutions.com.pk:44891/api/lookupapi/CustomerStatement?cardcode=';
const String newArrivalGuest='http://imtxt.sbsolutions.com.pk:44891/api/GetAllNewArrivalsGuest';
const String newArrivalAuth='http://imtxt.sbsolutions.com.pk:44891/api/GetAllNewArrivalsAuth?code=all';
const String contactUsUrl = 'http://sap.al-imdad.com:44891/Picture/CustomerApplogin/Contact.json';
const String allProductsUrl='http://imtxt.sbsolutions.com.pk:44891/api/GetAllItems';
const String allProductGuestUrl='http://imtxt.sbsolutions.com.pk:44891/api/GetAllItemsGuest';
const String categoryProductUrl='http://imtxt.sbsolutions.com.pk:44891/api/GetAllCategoryItems?Code=';
const String productAuthUrl='http://imtxt.sbsolutions.com.pk:44891/api/GetAllItemsAuth?code=';
const String faqUrl = 'http://sap.al-imdad.com:44891/Picture/CustomerApplogin/faq.json';

const String feedBackUrl = 'http://imtxt.sbsolutions.com.pk:44891/api/PostApis/PostAppFeedback';

const String sliderImagesUrl = 'http://imtxt.sbsolutions.com.pk:44891/api/Get/Slider';


const String surveyQuestionUrl = 'http://imtxt.sbsolutions.com.pk:44891/api/lookupapi/GetAllQuestions?CardCode=C00459';