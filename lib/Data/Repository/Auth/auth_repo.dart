...
class AuthRepo
{
  Future<dynamic> loginToken(body)async{
    try{
      ...
      return await Api.post("http://imtxt.sbsolutions.com.pk:44891/token",body,header: headers).then((value){
        if (kDebugMode) {
    ...
    print('value1');
      print(value1['body']);
        if(value1['access_token']!=null) {
          ...
          return await Api.getCat('http://imtxt.sbsolutions.com.pk:44891/api/lookupApi/LoginSAPCustomer?CustomerCode=$username&password=$password').then((value){
            if (kDebugMode) {
