  ...
  Widget build(BuildContext context) {
    return BlocConsumer<CategoryCubit, CategoryState>(
      ...
      builder: (context, state) {
        if (state is CategoryLoaded) {
          print("${state.categoryData.length}data cat");
          return SizedBox(
            height: 115,
            width: 1.sw,
...
// Utils.productTitle.value=Utils.categoryDummyProduct[index].productName.toString();
                      ...
                      Navigate.to(
                          context,
                          ProductScreen(
                              title: state.categoryData[index].cat.toString(),
                              isGuest: widget.isGuest,
                              catId:
                                  state.categoryData[index].catId.toString()));
                      ...
                      child: Column(
                        children: [
                          Expanded(
                            ...
                            child: CachedImage(
                              url:
                                  "$categoryImageUrl${state.categoryData![index].catImgJpg!}",
                            ),
                          ),
                          CustomSizedBox.height(5),
                          Expanded(
                            child: AppText(
                                state.categoryData[index].cat.toString(),
                                maxLine: 2,
...
//color: index==0?Colors.red:Colors.black,
                        child: Column(
                          children: [
                            AssetImageWidget(
                              url: Utils
                                  .categoryDummyProduct[index].productImage
                                  .toString(),
                              isCircle: true,
                              radius: 35.sp,
                            ),
