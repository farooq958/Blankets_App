...
//   }
          ...
          emit(StatementError(error: value['error']));
        }
      }).catchError((e) {
        throw e;
  ...
  Map<String, List<StatementApiModel>> groupStatementsByMonthYear(
      List<StatementApiModel> statements) {
    ...
    for (StatementApiModel statement in statements) {
      if (statement.refDate != null) {
        ...
        DateFormat("MM/dd/yyyy").parse(statement.refDate.toString());

        // Create a key in the format "MMM yyyy" (e.g., "Jan 2023") for the month and year
        String key =
            "${statementDate.month.gregorianMonthName.toString().padLeft(
            2, '0')} ${statementDate.year}";
  ...
  List<StatementApiModel> filterStatementsByDate(
      List<StatementApiModel> statements,
      DateTime startDate,
      DateTime endDate) {
  ...
  List<StatementData> getActualData(
      Map<String, List<StatementApiModel>> groupedData) {
    ...
    for (var dto in groupedData.entries) {
      ...
      returnDto.add(StatementData(dto.key, "",
          "Rs ${creditTotal.toString()} Cr", "Rs ${debitTotal.toString()} Dr"));
      for (var subDto in dto.value) {
//print();
        DateTime dt = DateFormat("MM/dd/yyyy")
            .parse(subDto.refDate
            .toString()
            .split(' ')
            .first);
        var value = subDto.source == 'Payment' ||
            subDto.source == 'JE' ||
            subDto.source == 'CR-NOTE'
            ? (double.parse(subDto.debit.toString()) >
            double.parse(subDto.credit.toString())
            ? '${subDto.debit.toString()} Dr'
            : '${subDto.credit.toString()} Cr')
            : '${subDto.debit.toString()} Dr';
        returnDto.add(StatementData(
            '${dt.day} ${dt.month.gregorianMonthName}, ${dt.year}',
            subDto.source.toString(),
            subDto.lineMemo.toString(),
            value));
      }
    }
    return returnDto;
